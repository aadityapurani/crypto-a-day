...
money = random.randint(1000000, 5000000)        # money in our wallet will be less than flag cost
signkey = ''.join([random.choice(string.letters+string.digits) for _ in xrange(random.randint(8,32))]) # btwn 8 and
items = [
...
def pay():
    ...
    sign = payment[sp+6:]
    payment = payment[:sp]
    signchk = sha256(signkey+payment).hexdigest()       # Signature validation
    if signchk != sign:
        print 'Invalid Order!'
        return
    ...
    if product == 'FLAG':
        print 'Good job! Here is your flag: flag_is_leet_testing_skillz'
